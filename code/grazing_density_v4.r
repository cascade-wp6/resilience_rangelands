...
pdf("C:\\Users\\SCHNEIDER\\Documents\\projects\\CAS01_grazing\\figures\\concept.pdf", width = 9, height = 4.5, paper = "special")

par(mfrow = c(2,3), mar = c(2,1,0,1),oma = c(2, 2, 2,2))

plot(NA,NA, xlim = c(0,1), ylim = c(0,.12), ylab = "deaths per timestep [ / timestep]", xlab = "vegetation cover", xaxt = "n", yaxs = "i" , xaxs = "i", yaxp = c(0, 0.1, 2))
#lines(rep(parms_temp$m, times = length(n))*dens ~ dens, data = result, col = "grey80", lwd = 2)

## intrinsic mortality 
abline(a = 0, b = parms_temp$m, lty = 3)
abline(a = 0, b = parms_temp$m+parms_temp$g/0.5, lty = 1)
abline(a = parms_temp$g, b = parms_temp$m , lty = 1, col = "red")

points(I((parent+parms_temp$m)*dens) ~ dens, data = result, col = "black", cex = 1, pch = 20)
points(I((stock+parms_temp$m)*dens) ~ dens, data = result, col = "red", cex = 1, pch = 20)


plot(NA,NA, xlim = c(0,1), ylim = c(0,.12), ylab = "deaths per timestep [ / timestep]", xlab = "vegetation cover", xaxt = "n", yaxt = "n", yaxs = "i" , xaxs = "i")
#lines(rep(parms_temp$m, times = length(n))*dens ~ dens, data = result, col = "grey80", lwd = 2)

k4 <- with(result, I((assoc3_4n)*n_4/(width*height)) ) 
k3 <- with(result, I((assoc3_3n)*n_3/(width*height)) ) 
k2 <- with(result, I((assoc3_2n)*n_2/(width*height)) ) 
k1 <- with(result, I((assoc3_1n)*n_1/(width*height)) ) 
k0 <- with(result, I((assoc3_0n)*n_0/(width*height)) ) 

tone <- colorRampPalette(c("white","darkgreen"))
cols <- tone(6)[c(2:6)]

polygon(result$dens*parms_temp$m+c(k0+k1+k2+k3,0,0) ~ c(result$dens,max(result$dens),min(result$dens)),  col = cols[4], border = FALSE)
polygon(result$dens*parms_temp$m+c(k0+k1+k2,0,0) ~ c(result$dens,max(result$dens),min(result$dens)),  col = cols[3], border = FALSE)
polygon(result$dens*parms_temp$m+c(k0+k1,0,0) ~ c(result$dens,max(result$dens),min(result$dens)),  col = cols[2], border = FALSE)
polygon(result$dens*parms_temp$m+c(k0,0,0) ~ c(result$dens,max(result$dens),min(result$dens)),  col = cols[1], border = FALSE)
#polygon(result$dens*parms_temp$m-c(k1+k2,2,0,0) ~ c(result$dens,max(result$dens),min(result$dens)),  col = cols[5], border = FALSE)
...
plot(NA,NA, xlim = c(0,1), ylim = c(0,.25), ylab = "deaths per timestep [ / timestep]", xlab = "vegetation cover", xaxt = "n", yaxt = "n", yaxs = "i" , xaxs = "i")
#lines(rep(parms_temp$m, times = length(n))*dens ~ dens, data = result, col = "grey80", lwd = 2)
...
tone <- colorRampPalette(c("white","red3"))
cols <- tone(6)[c(2:6)]
points(I((stockassoc3_4n+parms_temp$m)*dens) ~ dens, data = result, col = cols[5], pch = 20)
points(I((stockassoc3_3n+parms_temp$m)*dens) ~ dens, data = result, col = cols[4], pch = 20)
points(I((stockassoc3_2n+parms_temp$m)*dens) ~ dens, data = result, col = cols[3], pch = 20)
points(I((stockassoc3_1n+parms_temp$m)*dens) ~ dens, data = result, col = cols[2], pch = 20)
points(I((stockassoc3_0n+parms_temp$m)*dens) ~ dens, data = result, col = cols[1], pch = 20)
...
plot(NA,NA, xlim = c(0,1), ylim = c(0,.5), ylab = "individual risk", xlab = "vegetation cover", yaxs = "i" , xaxs = "i", xaxp = c(0, 1, 2), yaxp = c(0, 0.5, 2))
#lines(rep(parms_temp$m, times = length(n))*dens ~ dens, data = result, col = "grey80", lwd = 2)

abline(a = parms_temp$m, b = 0 , lty = 3)
abline(a = parms_temp$m+parms_temp$g/0.5, b = 0, lty = 1)

x <- seq(0,1, length = 50)
lines(x, parms_temp$m + parms_temp$g/x, lty = 1, col = "red")

points(I((parent+parms_temp$m)) ~ dens, data = result, col = "black", cex = 1, pch = 20)
points(I((stock+parms_temp$m)) ~ dens, data = result, col = "red", cex = 1, pch = 20)



plot(NA,NA, xlim = c(0,1), ylim = c(0,.5), ylab = "individual risk", xlab = "vegetation cover", yaxt = "n", yaxs = "i", xaxp = c(0, 1, 2) , xaxs = "i")
#lines(rep(parms_temp$m, times = length(n))*dens ~ dens, data = result, col = "grey80", lwd = 2)
...
points(I(parms_temp$m+(assoc3_4n/dens) ) ~ dens, data = result, col = cols[5], pch = 20)
points(I(parms_temp$m+(assoc3_3n/dens) ) ~ dens, data = result, col = cols[4], pch = 20)
points(I(parms_temp$m+(assoc3_2n/dens) ) ~ dens, data = result, col = cols[3], pch = 20)
points(I(parms_temp$m+(assoc3_1n/dens) ) ~ dens, data = result, col = cols[2], pch = 20)
points(I(parms_temp$m+(assoc3_0n/dens) ) ~ dens, data = result, col = cols[1], pch = 20)
...
points(I(parms_temp$m+(stockassoc3_4n/dens) ) ~ dens, data = result, col = cols[5], pch = 20)
points(I(parms_temp$m+(stockassoc3_3n/dens) ) ~ dens, data = result, col = cols[4], pch = 20)
points(I(parms_temp$m+(stockassoc3_2n/dens) ) ~ dens, data = result, col = cols[3], pch = 20)
points(I(parms_temp$m+(stockassoc3_1n/dens) ) ~ dens, data = result, col = cols[2], pch = 20)
points(I(parms_temp$m+(stockassoc3_0n/dens) ) ~ dens, data = result, col = cols[1], pch = 20)
...
plot(NA,NA, xlim = c(0,1), ylim = c(0,.25), ylab = "deaths per timestep [ / timestep]", xlab = "vegetation cover", xaxt = "n", yaxs = "i" , xaxs = "i")
#lines(rep(parms_temp$m, times = length(n))*dens ~ dens, data = result, col = "grey80", lwd = 2)

abline(a = 0, b = parms_temp$m, lty = 3)
abline(a = 0, b = parms_temp$m+ parms_temp$g/2, lty = 1)

abline(a = parms_temp$g, b =0,  col = "red",lty = 1)

lines(I(parent*dens) ~ dens, data = result, col = "black", lwd = 3)

lines(I(stock*dens) ~ dens, data = result, col = "red", lwd = 3)
...
lines(I(n_0) ~ dens, data = result, col = "black", lwd = 2)
