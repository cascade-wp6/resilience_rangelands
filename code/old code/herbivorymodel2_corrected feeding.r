...
# 4 - applying the rules to fill the cells in x_new
		...
		if(any(c(recolonisation+degradation, death, regeneration) > 1 )) stop(paste("a set probability is exceeding 1 in time step", i, "! decrease delta!!!")) 
...
# 5 saving state of the new grid	
		...
		result$mortality[i-1] <- mean(death[x_old$cells == "+"], na.rm = TRUE)/delta
...
result$mortality[i] <- mean(death[x_new$cells == "+"], na.rm = TRUE)/delta
...
windows(10,5)
pdf("grazing_corrected3.pdf", width = 12, height = 6, paper = "special" )
...
par(mar = c(2.5,4,2,1)+.1, xaxt = "s", yaxt ="s")
plot(result$time, result$mortality, pch = 19, lwd = 2, ylim = c(0.08,1.02))
dev.off()
...
saveGIF( 
	for(i in seq(1, length(result$timeseries), 1/delta*2)) {
		...
		vul <- sum(count(x, c("0", "-"), "+" ))
		feeding <- (parameters$g/vul*count(x, c("0", "-"), "+")/4 )
		breaks <- 10^seq(  -7, log10(parameters$g ),length = 11)

		x$cells <- as.numeric(x$cells)+11
		x$cells[x$cells == 12] <- findInterval(feeding, breaks)+1
		x$cells <- as.factor(x$cells)
		levels(x$cells) <- c(levels(x$cells), as.character(which(! as.character(1:14) %in% levels(x$cells))))
		plot(x, grid = FALSE, cols = grazingcol[as.numeric(levels(x$cells))], ani = TRUE)
		}
	}
, movie.name = "herbivory_corrected.gif", img.name = "grid", convert = "convert", interval = 0.01/1,
