...
#setwd("E:\\Eigene Dokumente\\Uni\\projects\\2013 CASCADE grazing\\sim10\\")
...
for(i in sort(unique(output$g))[-c(1)] ) {
...
pdf("E:\\SkyDrive\\Uni\\projects\\2013 Grazing models (CASCADE)\\figures\\stability.pdf", height = 7, width = 8, paper = "special")
...
layout(rbind( cbind(A, c(0,0,0), A+12), rep(0, times = 9), cbind(A+24, c(0,0,0), A+36) ) , width = c(1,1,1,1,0.4,1,1,1,1), height = c(1,1,1,0.4,1,1,1))
par(oma = c(4,4,1,1)+.1, mar = c(0,0,0,0))
#layout.show(n = 48)

for(i in 1:4) {

	#with(output[output$g == grazing & output$stock == c(FALSE, FALSE, TRUE, TRUE)[i] & output$globalgrazing == c(TRUE, FALSE, TRUE, FALSE)[i],], {
	
		for(j in c(0.5 , 0.7, 0.90 )) {
			for(k in unique(output$g)[-1]) {
				try({
				iteration = output$ID[which(output$g == k & output$b == j)]
							
				load(paste("results\\result", iteration[i], sep = "_"))
				
				extract <- as.vector(matrix(1:10000, ncol = 100, byrow = TRUE)[1:100,1:100])
	...
	#	mtext(b, c(0.4,0.47,0.54, 0.6), labels = best, col = "white")
	#}
	#)
}
...
pdf("C:\\Users\\SCHNEIDER\\SkyDrive\\Uni\\projects\\2013 Grazing models (CASCADE)\\manuscript\\first draft\\figures\\models.pdf", height = 7, width = 8, paper = "special")
