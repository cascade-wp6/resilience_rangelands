...
init <- c( 0.3, 0.8 )  # 0.25, 0.9, 0.3, 0.8 , ,  seq(0.25, 1, .05)
global <- c(FALSE, TRUE)
stock <- c(FALSE, TRUE)
...
# pre-run of 1000 timesteps without grazing (g = 0) and high or low environemntal parameter. 
 ...
 parms_temp <- list(
		...
		b = parameters[parameters$ID == iteration,4] , 		# beta*eps 
		d = 0.1,		# degradation
		c_ = 0.2, 		# beta*g  
		del = 0.1,  	# seeds dispersed; (1-del) seeds on nearest neighbourhood
		r = 0.01, 	# regeneration rate
...
result <- list()   # create an output list and
			...
			result$q_[[j]][1]  <- mean(rowSums( sapply(interact, function(k) (x_0$cells == states[j])[x_with_border][x_to_evaluate[x_0$cells == "+"]+k]) )) /4# write initial local densities 
			}
...
#  initialising first iteration 
	...
	result$timeseries <- as.data.frame(matrix(c( parms_temp$ID, TRUE, FALSE, parms_temp$starting, 1, parms_temp$g, parms_temp$m, parms_temp$b, result$rho[[1]][1], result$q_[[1]][1], x_0$cells), nrow = 1, dimnames = list("1", header_grids)))
