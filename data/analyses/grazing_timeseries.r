...
#setwd("E:\\Eigene Dokumente\\Uni\\projects\\2013 CASCADE grazing")

env <- 	unique(c(seq(0.2,1,.02)) )#seq(0.2,1,.025)
...
snapshots <-c( timesteps-175:1*4, timesteps)/delta+1
...
iteration =  240 # 198  #219  240  #grazing intensity Kefimodel
iteration =  632 # 146 #272  398   20
set.seed(iteration)
...
# 5 saving state of the new grid		
		...
		if(i %in% snapshots[seq(76,176,10)]) {
...
save(result, file =  paste("grazing_", iteration, sep = "")) 
...
library(animation)
ani.options(ffmpeg = "C:\\R\\ffmpeg\\bin\\ffmpeg")
...
saveGIF( 
	for(i in seq(2, length(result$timeseries), 1)) {
	  	par(mar = c(0,0,0,0))
		grd <- result$timeseries[[i]]

if(parameters[iteration,]$global) {
	vul <- length(which(grd$cells == "+"))/(width*height)
	feeding <- rep((parameters[iteration,]$m + parameters[iteration,]$g/vul), times = length(which(grd$cells == "+")))  

} else {
	vul <- sum(4-count(grd, "+" )[grd$cells == "+"])/(width*height)
	feeding <- (parameters[iteration,]$m + parameters[iteration,]$g/vul*(4-count(grd, "+" ))[grd$cells == "+"] )
		...
		plot(grd, grid = FALSE, cols = grazingcol[as.numeric(levels(grd$cells))], ani = TRUE)
	
	}
, movie.name = paste("grazing_", iteration, ".gif", sep = ""), img.name = "grid", convert = "convert", interval = 0.01/1,
...
, video.name = paste("grazing_", iteration, ".mp4", sep = ""), img.name = "grid", interval = .1,
    cmd.fun = system, clean = TRUE, ani.width = width*5, ani.height = height*5, outdir = getwd())


ani.options(swftools = "C:\\R\\SWFTools")
...
, swf.name = paste("grazing_", iteration, ".swf", sep = ""), img.name = "grid", interval = .1,
    cmd.fun = system, clean = TRUE, ani.width = width*5, ani.height = height*5, outdir = getwd())
...
pdf(paste("grazing_", iteration, ".pdf", sep = ""), height = 6, width = 6, paper = "special")
par(mfrow = c(1,1), oma = c(0,0,2,0))

	plot(NA, NA, xlim = c(1,2500), ylim = c(.001,100), log= "xy", main = "undifferentiated grazing" )
	
	if(!any(is.na(result$cumpatch))) lines(I(mean)~size, pch = 20, col = "black", lwd = 2, data = result$cumpatch)
