...
# 1 - setting time-step parameters
	...
	# determine vulnerability of a random cell (summed unocupied neighbors over lattice size) 
		...
		if(parms_temp$global == FALSE) {
				...
				parms_temp$vul <- length(which(x_old$cells == "+"))/(width*height)
				if(parms_temp$vul == 0) parms_temp$vul = 1/(width*height)
			}
		# 168.6152  , 170.84
...
for(j in  graz) {
	...
	if(!is.na(result$cumpatch2)) lines(I(mean/max(mean))~size, pch = 20, col = color[i], lwd = 2, data = result$cumpatch2)
	}
...
pdf("patchdist_graz2.pdf", height = 9, width = 6, paper = "special")
par(mfrow = c(2,1), oma = c(0,0,2,0))
