...
snapshots <- c(1, c(addgrazing, timesteps-8:1*50, timesteps)/delta+1)
...
# 5 saving state of the new grid		
		...
		if(i %in% snapshots) {
				result$timeseries <- rbind(result$timeseries, 
								as.data.frame(matrix(c( iteration, TRUE, parms_temp$starting, (i-1)*delta, parms_temp$g, parms_temp$m, parms_temp$b,  x_new$cells), nrow = 1, dimnames = list("1", header_grids)))
				)
