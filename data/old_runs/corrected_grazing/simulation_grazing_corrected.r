...
setwd("C:\\Users\\SCHNEIDER\\Documents\\CASCADE\\2013_grazingmodel\\corrected_grazing")
env <- 	seq(0.2,1,.025)
graz <- seq(0,.5, 0.025)
...
snapshots <- c(1, c(addgrazing, timesteps-10:1*50, timesteps)/delta+1)
...
pdf("env_gradient.pdf", height = 4, width = 3, paper = "special")
par(mfrow = c(1,1), oma = c(0,0,2,0))
for(i in sort(unique(output$g)) ) {

	with(output[output$g_gr == i & output$grazing == TRUE,], 
		plot(b, rho_plus , ylim = c(0,1), xlim = c(0,1), pch = 20, col = "red3", main = paste("g = ", i)))
	
	with(output[output$g_gr == i & output$grazing == FALSE,], 
...
pdf("grazing_gradient.pdf", height = 4, width = 3, paper = "special")
par(mfrow = c(1,1), oma = c(0,0,2,0))
for(i in sort(unique(output$b)) ) {

	with(output[output$b == i & output$grazing == TRUE,], 
		plot(g_gr, rho_plus , ylim = c(0,1), xlim = c(0,.5), pch = 20, col = "red3", main = paste("b = ", i)))
	
	with(output[output$b == i & output$grazing == FALSE,], 
