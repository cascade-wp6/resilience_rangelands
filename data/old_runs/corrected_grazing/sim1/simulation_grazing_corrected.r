...
#  initialising first iteration 
	...
	#write.table(c(iteration, i, parms_temp$g, parms_temp$m, parms_temp$b, i$cells), "grids.csv", row.names = FALSE, col.names = FALSE, sep = ",", append = TRUE)
			...
			result$timeseries <- as.data.frame(matrix(c( parms_temp$ID, FALSE, parms_temp$starting, 1, parms_temp$g, parms_temp$m, parms_temp$b, parms_temp$g, parms_temp$m, initial$cells), nrow = 1, dimnames = list("1", header_grids)))
	

	parms_temp <- parameters_ini[[parameters[iteration,2]]]

if(!is.na(out$mortality)) for(i in 2:(timesteps/delta+1)) {    #calculation loop

		if(i == addgrazing/delta  ) {
			parms_temp <- as.list(parameters[iteration,])
			parms_temp$m <- out$mortality
			parms_temp$g <- 0
		}
...
# 5 saving state of the new grid		
		...
		if(i %in% snapshots) result$timeseries <- rbind(result$timeseries, 
								as.data.frame(matrix(c( parms_temp$ID, FALSE, parms_temp$starting, i, parms_temp$g, parms_temp$m, parms_temp$b, parms_temp$g, parms_temp$m, x_new$cells), nrow = 1, dimnames = list("1", header_grids)))
	)
...
pdf("env_gradient.pdf", height = 4, width = 11, paper = "special")
par(mfrow = c(1,3), oma = c(0,0,2,0))
for(i in sort(unique(output$g)) ) {
	for(j in sort(unique(output$m0[output$grazing == TRUE]))) {	
	with(output[output$g_gr == i & output$grazing == TRUE & output$m0_gr == j,], 
		plot(b, rho_plus , ylim = c(0,1), xlim = c(0,1), pch = 20, col = "red3", main = paste("m0 =", j)))
	
	with(output[output$g_gr == i & output$grazing == FALSE & output$m0_gr == j,], 
		points(b, rho_plus, pch = 20, col = "black" ))
	}
	mtext(paste("g = ", i), 3, outer = TRUE)
}
dev.off()



pdf("grazing_gradient.pdf", height = 4, width = 11, paper = "special")
par(mfrow = c(1,3), oma = c(0,0,2,0))
for(i in sort(unique(output$b)) ) {
	for(j in sort(unique(output$m0[output$grazing == TRUE]))) {	
	with(output[output$b == i & output$grazing == TRUE & output$m0_gr == j,], 
		plot(g_gr, rho_plus , ylim = c(0,1), xlim = c(0,1), pch = 20, col = "red3", main = paste("m0 =", j)))
...
patchdist <- function(i) {
# sampling the initial random grid into a list object
grid1 <- list(  
	dim = c(as.integer(width), as.integer(height)),  # first element contains the dimensions of the landscape 
	cells = as.factor( as.integer(grids[i,-(1:9)]))  )
levels(grid1$cells) <- states  #assign cell states 
class(grid1) <- c("list","landscape") # set class of object (required for plotting)
...
for(j in  seq(0,.9, 0.15)) {
p <- unlist( lapply( with(grids,which(grazing==1 & timestep %in% snapshots[-1:2] & g == j & m == .1 & starting == 2 & b ==  0.350)), patchdist))
