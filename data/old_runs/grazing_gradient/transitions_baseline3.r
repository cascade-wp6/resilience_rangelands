...
# 5 saving state of the new grid		
		...
		if(i %in% snapshots) write.table(c(iteration, i, parms_temp$g, parms_temp$b, x_new$cells), "grids.csv", row.names = FALSE, col.names = FALSE, sep = ",", append = TRUE)
...
cols[as.integer((x-min(x))/(max(x)-min(x))*steps+1)]
...
with(data = output, plot(grazing, mortality, ylim = c(0,.3), xlim = c(0,.3), pch = 20, col = highlight(environmt, c("black", "green3")) ))
with(data = output, points(grazing, mortality_border, pch = 20, col = highlight(environmt, c("black", "red3")) ))

with(data = output, plot(mortality, rho_plus, pch = 20 , col =  highlight(environmt, c("black", "green3"))))
