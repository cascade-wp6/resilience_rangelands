...
env <- 	seq(0.2,1,length = 50)
graz <- c(0, seq(0.05, 0.25, length = 50))
...
replicates = 1
env <- rep(rep(env, times = lgraz), times = replicates)
graz <- rep(rep(graz, each = lenv), times = replicates)

snapshots <- c(1, 1000, 1750, 1800, 1850, 1900, 1950, 2000)/delta+1
...
with(data = output, plot(environmt, rho_plus, xlim = c(0,1), ylim = c(0,1), pch = 20, col = cols[as.integer((grazing-min(grazing))/(max(grazing)-min(grazing))*49+1)] ))
with(data = output, plot(environmt, q_plus, xlim = c(0,1), ylim = c(0,1), pch = 20, col = cols[as.integer((grazing-min(grazing))/(max(grazing)-min(grazing))*49+1)] ))
with(data = output, plot(environmt, mortality, xlim = c(0,1), ylim = c(0,1), pch = 20, col = cols[as.integer((grazing-min(grazing))/(max(grazing)-min(grazing))*49+1)] ))
with(data = output, plot(environmt, mortality_border, xlim = c(0,1), ylim = c(0,1), pch = 20, col = cols[as.integer((grazing-min(grazing))/(max(grazing)-min(grazing))*49+1)] ))
