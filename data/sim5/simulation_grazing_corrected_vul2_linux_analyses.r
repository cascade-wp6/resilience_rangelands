...
#setwd("C:\\Users\\SCHNEIDER\\Documents\\CASCADE\\2013_grazingmodel\\corrected_grazing_vul\\sim5")
#setwd("E:\\Eigene Dokumente\\Uni\\projects\\2013 CASCADE grazing\\sim5")

env <- 	unique(c(seq(0.2,1,.04)) )#seq(0.2,1,.025)
graz <- c(.05, .15, .25)*0.66  #c(0.0375, 0.1125, 0.1875)
#graz <- seq(0,.1, 0.005)#seq(0,.9, 0.075)
mort <- 0.05 #seq(0.1,.3, 0.1)
init <- c("low", "high")
global <- c(TRUE, FALSE)
stock <- c(TRUE, FALSE)
...
parameters = data.frame(
	ID = 1:(lgraz*lmort*lglobal*linit*lenv*lstock), 
	global = rep(global, each = lgraz*lenv*lmort*linit*lstock),
	stock =  rep(rep(stock, each = lgraz*lenv*lmort*linit), times = lstock),
	starting = rep(rep(init, each = lgraz*lenv*lmort), times = lglobal*lstock),
	m = rep(rep(mort, each = lgraz*lenv), times = lglobal*linit*lstock), 		# intrinsic mortality
	g = rep(rep(graz, each = lenv), times = lmort*lglobal*linit*lstock),		#grazing
	b = rep(env, times = lgraz*lmort*lglobal*linit*lstock), 		# beta*eps 
	d = 0.1,		# degradation
	c_ = 0.2, 		# beta*g  
...
workerlist <- rep("localhost", times = 1)
...
pdf("E:\\SkyDrive\\Uni\\talks\\env_gradient.pdf", height = 3.5, width = 9, paper = "special")
par(mfcol = c(1,3), oma = c(2,2,2,1), mar = c(2,2,0,0)+.1)

plot(NA,NA, xlim = c(0,1), ylim = c(0,1) )
plot(NA,NA, xlim = c(0,1), ylim = c(0,1), yaxt = "n" )
plot(NA,NA, xlim = c(0,1), ylim = c(0,1), yaxt = "n"  )


	for(i in graz) {
		plot(rho_plus ~ b, pch = 20, col = "#B2BE8D", xlim = c(0,1), ylim = c(0,1) , data = output[output$global == TRUE & output$stock == FALSE & output$g == i,], yaxt = "n", xaxt = "n")
	}
	for(i in graz) {				
		plot(rho_plus ~ b, pch = 20, col = "#91BACB", xlim = c(0,1), ylim = c(0,1), data = output[output$global == FALSE & output$stock == FALSE & output$g == i,], yaxt = "n", xaxt = "n")
	}
	for(i in graz) {
		plot(rho_plus ~ b, pch = 20, col = "#968376", xlim = c(0,1), ylim = c(0,1),  data = output[output$global == TRUE & output$stock == TRUE & output$g == i,], yaxt = "n", xaxt = "n")
}
	for(i in graz) {
		plot(rho_plus ~ b, pch = 20, col =  "#928A95", xlim = c(0,1), ylim = c(0,1),  data = output[output$global == FALSE & output$stock == TRUE & output$g == i,], yaxt = "n", xaxt = "n")
	}
...
pdf("E:\\SkyDrive\\Uni\\talks\\patchdist.pdf", height = 3.5, width = 9, paper = "special")
par(mfcol = c(1,3), oma = c(2,2,2,1), mar = c(2,2,0,0)+.1)
...
Q <- count(grd, "+" )/4
if(parameters[x,]$global) {
	if(parameters[x,]$stock) {vul <- length(which(grd$cells == "+"))/(width*height)} else{vul <- .66}
	if(vul == 0) vul = 1/(width*height)
	feeding <- rep((parameters[x,]$m + parameters[x,]$g/vul), times = length(which(grd$cells == "+")))  

} else {
	if(parameters[x,]$stock) {vul <- sum(4-count(grd, "+" )[grd$cells == "+"])/(width*height)} else{vul <-0.165}
	feeding <- (parameters[x,]$m + parameters[x,]$g/vul*(4-count(grd, "+" ))[grd$cells == "+"] )

}

		breaks <- seq(.05,.5, length = 31) #10^seq(  -1, log10(0.11 ),length = 11) #
...
output[output$b == 0.52 & output$starting == "high" & output$g == 0.033,]


pdf("E:\\SkyDrive\\Uni\\talks\\grids.pdf", width = 5, height = 5, paper = "special")
...
pdf("E:\\SkyDrive\\Uni\\talks\\patchdist.pdf", width = 5, height = 5, paper = "special")
par(mfrow = c(1,1), mar = c(4,4,2,1))

select <- c(386, 632, 140, 878)

	for(i in 1:length(select)) {
	load(paste("results/result_", select[i], sep = ""), envir = globalenv() )
	plot(NA, NA, xlim = c(1,10000), ylim = c(.001,1), log= "xy" , xlab = "size of patch", ylab = "number of patches")
	if(all(!is.na(result$cumpatch))) points(I(mean/max(mean))~size, pch = 20, data = result$cumpatch)
	}
...
output[output$b == 0.66 & output$starting == "high" & output$g == 0.033,]

pdf("E:\\SkyDrive\\Uni\\talks\\grids2.pdf", width = 5, height = 5, paper = "special")
par(mfrow = c(1,1), mar = c(1,1,1,1))
