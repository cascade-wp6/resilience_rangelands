...
global <- list(  
  ...
  d = 0.1,    # degradation
  c = 0.2, 		# beta*g  
  delta = 0.1,  	# seeds dispersed; (1-del) seeds on nearest neighbourhood
  r = 0.01, 	# regeneration rate
  f = 0.9		# local fascilitation
)

parameters <- list(
  b = seq(0,1,.01) #seq(0,1,.002)+0.001
)
...
foreach(iteration = iterations$ID, .combine = rbind, .packages = c("deSolve")) %dopar% { 
 model_parms <- as.list(iterations[iteration,])
  ...
  # running the ode-solver
  runmodel <- ode(y = rho, func = odesys, times = c(1,60), parms = model_parms)
