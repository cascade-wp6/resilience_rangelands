...
This is assuming that the local environmental suitability is enhanced by a local facilitation term similar to the implementation in Kéfi et al 2007. Thus, the reduction in growth rate, $r$, by aridity, $a$, is compensated by a function of $q_{0|1}$, the local density of cells in state 1 (i.e. vegetation) given that the focal cell is in state 0 (i.e. empty), and maximizes to one if the cell has four neighbors:
...
Mortality becomes enhanced in sparse environments by patches that are attracting grazers. We assume this attractivity enhances search efficiency locally by the value $v$
...
The vegetation dynamics of the system can be fully described by two differential equations (omitting indexed zeros of equations for easier reading): 

$$ \frac{\mathrm{d}\rho_1}{\mathrm{d}t} = r ( a + (f - a) q_{0|1} ) \rho_1^{1 + \alpha} (1-\frac{\rho_1}{K (1 - c q_{0|1})}) - m \rho_1 - \frac{(b  + v q_{1|1} )\rho_1^{1+q}L}{1+( b  + v q_{1|1} ) ( h  / (p  q_{1|1}))\rho_1^{1+q}} $$

$$ \frac{\mathrm{d}\rho_0}{\mathrm{d}t} =  m \rho_1 + \frac{(b  + v q_{1|1} )\rho_1^{1+q}L}{1+( b  + v q_{1|1} ) ( h  / (p  q_{1|1}))\rho_1^{1+q}} - r ( a + (f - a) q_{0|1} ) \rho_1^{1 + \alpha} (1-\frac{\rho_1}{K (1 - c q_{0|1})})$$

Next steps: 

- formulate equations for $\frac{\mathrm{d}\rho_{0,1}}{\mathrm{d}t}$, $\frac{\mathrm{d}\rho_{0,0}}{\mathrm{d}t}$, and $\frac{\mathrm{d}\rho_{1,1}}{\mathrm{d}t}$
