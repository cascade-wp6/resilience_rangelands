...
d_rho <- function(rho, parms, z = 4) { 
  ...
  } # instead use function from simfunctions.r?
  
# running the ode-solver
ODEglobal_high <- ode(y = 0.9, func = odesys, times = 1.1^(0:50)-1, parms = parms)
...
# running the ode-solver
ODEglobal_low <- ode(y = 0.1, func = odesys, times = 1.1^(0:50)-1, parms = parms)
...
```{r, eval = FALSE, dev='png', echo = FALSE, fig.show='animate', fig.height = 4, fig.width = 4, interval = 0.01}
if(Sys.info()[['sysname']] == "Linux") X11.options(antialias = "none") #for Linux Systems to enable pixel-wise plotting in (animated) gif-files. 
  ...
  width = nullmodel$timeseries[[1]]$dim[1]
  height = nullmodel$timeseries[[1]]$dim[2]
     
    for(i in 1:length(nullmodel$timeseries) ) {
      par(mar = c(0,0,0,0))
      plot(nullmodel$timeseries[[i]], grid = FALSE, ani = TRUE) 
    }
```
...
If those effects coincide, potentially even with the global feedbacks, the intersections are difficult to estimate visually. 
...
$$ C = \rho_{1}m + \frac{(a  + v \rho_1)(1 - p \rho_1)L \rho_1^{1+q}}{1+(a  + v \rho_1)h\rho_1^{1+q}}$$
...
parms <- list(
    ...
    a = 0.2, # search efficiency
    v = 0.8, # attraction effect
    h = 50, # handling time (one individual on landscape unit)
    p = 0.99, # protection effect
...
Visualising the attractor of the system is not as straightforward as for the mean-field models that are determined by global cover only, where the intersections of growth and mortality defined the equilibria. In the spatially-explict pair-approximation model the attractor can be visualised if we assume that clustering in the initial state is non-random, and we assign starting values for $\rho_1$ and global mortality of the population $m_1$. 

I assign inital values of mortality equally along the x-axis for the assumption of highest and lowest clustering. The development of global and local cover yields timeseries of average growth and mortality for all those virtualised cases which can be plotted as arrowstowards the attractors. 

```{r, echo=FALSE, warning=FALSE}
attractor(parms)
```

This nicely illustrates that the unstable equilibrium now turns from a simple threshold point to a line or surface. 

### Bifurcation diagrams

The following shows bifurcation diagrams of the mean field approximation over an environmental gradient, $b$, as well as over a gradient of livestock density, $L$. 

```{r, echo=FALSE, fig.height = 3, warning=FALSE }
par(mfrow = c(1,2))
parms$q = 0.4
bifurcation(parms, "L", c(0,12), returnoutput = TRUE) -> over_L
 
bifurcation(parms,"b", c(0,1), returnoutput = TRUE) -> over_b
...
The same graph using the pair-approximation. 

```{r, echo=FALSE, fig.height = 3, warning=FALSE}
par(mfrow = c(1,2))
bifurcation(parms, "L", pairapprox = TRUE, meanfield = FALSE, c(0,12), returnoutput = TRUE) -> over_L
 
bifurcation(parms,"b", pairapprox = TRUE, meanfield = FALSE, c(0,1), returnoutput = TRUE) -> over_b


```

The bifurcation diagram visualises the attractor only along the total cover which is only an incomplete visualisation, since the steady state is described by both, global vegetation cover $\rho_1$ and average local cover $q_{1|1}$ (or alternatively the cover of vegetated pairs $\rho_{11}$ but this is rather unintuitive). To visualise this, a three dimensional graph is necessary. 
...
persp(0:1,0:1, matrix(c(0,0,0,0), ncol = 2), zlim = c(0,1), ticktype = "detailed", zlab = "vegetation cover", ylab = "local cover" , xlab = "b", theta = -65, phi = 19, r = sqrt(3), d = 1) -> p
  ...
  points(trans3d(over_b$pairapprox$b, over_b$pairapprox$rho_1, rep(0, length(over_b$pairapprox$rho_1)), pmat = p), col = "#D0D0D0", pch = 20,  cex = 0.5)
  points(trans3d(over_b$pairapprox$b,rep(1, length(over_b$pairapprox$rho_1)), over_b$pairapprox$rho_11/over_b$pairapprox$rho_1,  pmat = p), col = "#D0D0D0", pch = 20,  cex = 0.5)  
  points(trans3d(rep(1, length(over_b$pairapprox$rho_1)),over_b$pairapprox$rho_1, over_b$pairapprox$rho_11/over_b$pairapprox$rho_1,  pmat = p), col = "#D0D0D0", pch = 20,  cex = 0.5)  
  
  points(trans3d(over_b$pairapprox$b, over_b$pairapprox$rho_1, over_b$pairapprox$rho_11/over_b$pairapprox$rho_1, pmat = p), pch = 20,  cex = 0.5)


```

Decomposing it into its dimensions is easier to print. 
...
par(mfrow =c(1,2))
  plot(over_b$pairapprox$b, over_b$pairapprox$rho_1, 
       pch = 20,  cex = 0.5, ylim = c(0,1), xlim = c(0,1),
       xlab = "environmental quality", ylab = "global cover"
       )
  plot(over_b$pairapprox$b, over_b$pairapprox$q_11, 
       pch = 20,  cex = 0.5, ylim = c(0,1), xlim = c(0,1),
       xlab = "environmental quality", ylab = "local cover")
```

The direct comparison of global cover vs. local cover indicates the amount of clustering along the stress gradient and at the critical thresholds. 
We are going to compare these plots for a range of models to see how the spatial configuration changes with increasing stress under different mechanism scenarios. 

```{r, echo=FALSE, fig.width = 3, fig.height = 3, warning=FALSE}
par(mfrow = c(1,1), mar = c(4,4,1,1))
  plot(over_b$pairapprox$rho_1, over_b$pairapprox$q_11, 
       pch = 20,  cex = 0.5, ylim = c(0,1), xlim = c(0,1),
       xlab = "global cover", ylab = "local cover")
