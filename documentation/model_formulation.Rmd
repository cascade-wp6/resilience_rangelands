...
In the cellular automata model and mean-field approximations of Sonia, individual plant mortality is a constant rate, which means it is increasing linearly with vegetation cover. 
...
```{r, echo=FALSE}
C <- function(V, a = 0.5, h = 10) (a*V)/(1+a*h*V)
rho <- seq(0,1,length = 100)
...
```{r, echo=FALSE}
C <- function(V, a = 1, h = 10, q = 1) (a*V^(1+q))/(1+a*h*(V^(1+q)) ) 
rho <- seq(0,1,length = 100)
